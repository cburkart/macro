...
getSymbols(c('GDPC96','GDPPOT','USACPIBLS','CPIAUCSL'),src='FRED')

CPI <- get('CPIAUCSL')
anchor <- as.numeric(last(CPI))
GDPC96.base <- mean(CPI['2009'])
GDPPOT.base <- mean(CPI['2009'])

gdp.ts <- ts(GDPC96['1949::']*(anchor/GDPC96.base),start=1949,frequency=4)
potgdp.ts <- ts(GDPPOT['1949::2015']*(anchor/GDPPOT.base),start=1949,frequency=4)
...
new.df <- data.frame(seq(from=1955,to=2015),as.numeric(window(ann.ytilde.ts,start=1955,end=2015)), as.numeric(window(deltapi.ts,start=1955,end=2015)))
names(new.df) <- c('year','ytilde','deltapi')

model.lm <- lm(deltapi~-1+ytilde,data=new.df)

plot(new.df$ytilde,new.df$deltapi,pch=21,col='blue',bg='yellow',main='Empricial Phillips curve, U.S. 1955-2013',xlab=expression('Short-run output, '*tilde(Y))
